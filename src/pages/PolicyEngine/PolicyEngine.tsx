...
const detectConflicts = (rules: PolicyRule[]): Record<string, string[]> => {
  const conflicts: Record<string, string[]> = {};
  for (let i = 0; i < rules.length; i++) {
    for (let j = i + 1; j < rules.length; j++) {
      const rule1 = rules[i];
      const rule2 = rules[j];
      // Simple conflict: same attributes and opposite effects with high priority gap
      if (
        rule1.effect !== rule2.effect &&
        Math.abs(rule1.priority - rule2.priority) < 10 &&
        rule1.conditions.some(c1 => rule2.conditions.some(c2 => c1.attribute === c2.attribute))
      ) {
        if (!conflicts[rule1.id]) conflicts[rule1.id] = [];
        conflicts[rule1.id].push(rule2.name);
      }
    }
  }
  return conflicts;
...
function PolicyEngineComponent() {
  ...
  const [traceStepAnimations, setTraceStepAnimations] = useState<boolean>(false);
  ...
  const evaluatePDP = () => {
    ...
    // Trigger trace animations
    setTimeout(() => setTraceStepAnimations(true), 100);
  };
      ...
      <div className="pe-trace-waterfall">
        {trace.map((step, idx) => (
          <div
            ...
            style={{ animationDelay: traceStepAnimations ? `${idx * 80}ms` : '0ms' }}
          >
            <div className={`pe-step-circle pe-step-circle-${step.result ? 'pass' : 'fail'}`}>
              <span className="pe-step-number">{step.step}</span>
            </div>
        ...
        <div className="pe-rules">
          <div className="pe-section">
            ...
            {Object.keys(conflicts).length > 0 && (
              <div className="pe-conflict-banner">
                <span className="pe-conflict-icon">⚠️</span>
                <span className="pe-conflict-text">
                  {Object.values(conflicts).flat().length} potential policy conflicts detected · Review priority levels
                </span>
              </div>
            )}
            ...
            {rules.map(rule => (
              <div key={rule.id} className="pe-rule-card">
                  ...
                  <div className="pe-rule-actions">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setConflicts(detectConflicts(rules.filter(r => r.id !== rule.id)));
                      }}
                    >
                      🗑️
                    </button>
                ...
                {conflicts[rule.id] && (
                  <div className="pe-conflict-notice">
                    <strong>Conflict with:</strong> {conflicts[rule.id].join(', ')}
                  </div>
                )}
              ...
              <div className="pe-section">
                ...
                <div className={`pe-decision-box pe-decision-${simulatorResult.decision.toLowerCase()}`}>
                  ...
                  <div className="pe-decision-info">
                    ...
                    <div className="pe-decision-meta">
                      <span className="pe-exec-time">⏱ {simulatorResult.executionTime}ms</span>
                ...
                <div className="pe-trace-section">
                  <h5>📋 Decision Trace (Step-by-Step Evaluation)</h5>
